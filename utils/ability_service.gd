...
func init_stats(from_stats:Resource, effects:Array) -> Resource:
    ...
    new_stats.damage = max(1, round(current_stats.damage * percent_dmg_bonus)) as int

    new_stats.cooldown = current_stats.cooldown

    new_stats.max_range = max(MIN_RANGE, current_stats.max_range + Utils.get_stat("stat_range")) as int
